<template>
    <div>
        ...
        <div v-for="(value, name) in allPermissions"
            :key="name"
            v-bind:class="[ hiddenModule.indexOf(name) != -1 ? 'hidden' : '' ]"
        >
            <h3>{{ name }}</h3>
            <div v-for="(permissionActions, subPermissionTitle) in value" :key="subPermissionTitle">
                <div class="row">
                    <div class="col-md-12">
                        <div class="row">
                            <div class="col-md-3">
                                <!-- <h4 class="pull-left">{{ trans('roles.module.'+subPermissionTitle) }}</h4> -->
                                <h4 class="pull-left">{{ subPermissionTitle }}</h4>
                            </div>
                            <div class="col-md-9">
    ...
    export default {
        ...
        data() {
            return {
                permissions: {},
                allPermissions: {},
                hiddenModule: ['Workflow','Workshop','Translation','Tag','Setting','Page','Menu','Media','Core','Dashboard','History'],
            };
        },
        methods: {
            triggerEvent() {
            ...
            setDefaultPermission() {
                // console.log(3)
                for(var name in this.allPermissions) {
                    if(this.hiddenModule.indexOf(name) != -1){
                        for(var subPermissionTitle in this.allPermissions[name]) {
                            for(var permissionTitle in this.allPermissions[name][subPermissionTitle]) {
                                this.permissions[subPermissionTitle+'.'+permissionTitle] = 1;
                            }
                            this.changeState(subPermissionTitle, this.allPermissions[name][subPermissionTitle], 1)
                        }
                    }
                }
            }
